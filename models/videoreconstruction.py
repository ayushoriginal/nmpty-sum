...
from ..layers import TextEncoder, ReverseVideoDecoder, VideoEncoder
...
class VideoReconstruction(nn.Module):
    ...
    def __init__(self, opts):
        ...
        # Textual context size is  equal to enc_dim * 2 when using bidirectional RNN
    ...
    def forward(self, batch, **kwargs):
        ...
        """
        # reversing the timesteps index in the batch 
        video_feats = batch['feats_in']
        idx = [i for i in range(video_feats.size(1)-1, -1, -1)]
        idx = Variable(torch.LongTensor(idx)).cuda()
        inverted_feats = video_feats.index_select(1, idx).transpose(0, 1)
       
        result = self.dec(self.encode(batch), inverted_feats) 
