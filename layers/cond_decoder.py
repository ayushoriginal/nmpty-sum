...
class ConditionalDecoder(nn.Module):
    ...
    def f_next(self, ctx_dict, y, h):
        ...
        if isinstance(coverage, int):
            self.coverage = self.txt_alpha_t
            self.cov_loss = 0
        else:
            self.coverage += self.txt_alpha_t
            self.cov_loss = torch.sum ( torch.min (self.coverage, self.txt_alpha_t) )
    ...
    def forward(self, ctx_dict, y):
        ...
        cc_loss = self.cov_loss
        print("COV LOSS: ", cc_loss)
        return {'loss': loss, 'logps': logps, 'cov_loss' : cc_loss }
